...
app.post("/rutas", async (req, res) => {
  ...
  try {
    ...
    const values = puntos.map((punto_id, index) => `($${index * 3 + 2}, $${index * 3 + 3}, $${index * 3 + 4})`).join(", ");
    const queryParams = puntos.flatMap((punto_id, index) => [nuevaRuta.id, punto_id, index + 1]);
